...
public class DecisionTreeInSpark() : Serializable {
    ...
    public fun simpleDecisionTree(trainData: JavaRDD<LabeledPoint>, cvData: JavaRDD<LabeledPoint>) {

        // Build a simple default DecisionTreeModel
        val model = DecisionTree.trainClassifier(trainData, 7, mapOf<Int, Int>(), "gini", 4, 100)
    ...
    public fun evaluate(trainData: JavaRDD<LabeledPoint>, cvData: JavaRDD<LabeledPoint>, testData: JavaRDD<LabeledPoint>) {
        val evaluations =
                listOf("gini", "entropy").flatMap { impurity ->
                    intArrayOf(1, 20).flatMap { depth ->
                        intArrayOf(10, 300).map { bins ->
                            val model = DecisionTree.trainClassifier(
                                    trainData, 7, mapOf<Int, Int>(), impurity, depth, bins)
        ...
        val model = DecisionTree.trainClassifier(
                trainData.union(cvData), 7, mapOf<Int, Int>(), "entropy", 20, 300)
